...
var stacked_inline_default = 'var I=Object.hasOwnProperty;function h(){return typeof document>"u"?"":document.body?.dataset?.basepath??""}function o(u,D){let n=D??h(),F=u.startsWith("/")?u:"/"+u;return n+F}var B="stacked-pages-state";function d(){let u=window.location.pathname;return u.startsWith("/")&&(u=u.slice(1)),u.endsWith("/")&&(u=u.slice(0,-1)),u||"index"}function m(){return document.querySelector("h1")?.textContent?.trim()||document.title||d()}function E(){try{let u=sessionStorage.getItem(B);if(u){let D=JSON.parse(u);if(Array.isArray(D.tabs)&&typeof D.activeIndex=="number")return D}}catch{}return{tabs:[],activeIndex:-1}}function C(u){try{sessionStorage.setItem(B,JSON.stringify(u))}catch{}}function f(u){return{maxTabs:parseInt(u.dataset.maxTabs||"8",10),mobileBreakpoint:parseInt(u.dataset.mobileBreakpoint||"800",10),showSpines:u.dataset.showSpines!=="false",animate:u.dataset.animate!=="false"}}function A(u,D){let n=f(u);if(window.innerWidth<n.mobileBreakpoint){u.style.display="none",document.body.classList.remove("has-binder-left","has-binder-right");return}if(u.style.display="",D.tabs.length<=1){u.innerHTML="",u.classList.remove("binder-active"),document.body.classList.remove("has-binder-left","has-binder-right");return}u.classList.add("binder-active");let F=D.tabs.slice(0,D.activeIndex+1),r=D.tabs.slice(D.activeIndex+1);if(u.innerHTML="",F.length>0){let t=document.createElement("div");t.className="binder-strip binder-strip-left";for(let e=0;e<F.length;e++){let i=F[e];t.appendChild(c(i,e,"left",D,n))}u.appendChild(t)}if(r.length>0){let t=document.createElement("div");t.className="binder-strip binder-strip-right";for(let e=0;e<r.length;e++){let i=D.activeIndex+1+e,s=r[e];t.appendChild(c(s,i,"right",D,n))}u.appendChild(t)}document.body.classList.toggle("has-binder-left",F.length>0),document.body.classList.toggle("has-binder-right",r.length>0)}function c(u,D,n,F,r){let t=document.createElement("div");if(t.className=`binder-tab binder-tab-${n}`,t.dataset.index=String(D),D===F.activeIndex&&t.classList.add("binder-tab-active"),r.showSpines){let i=document.createElement("div");i.className="binder-spine",t.appendChild(i)}let e=document.createElement("span");if(e.className="binder-label",e.textContent=u.title,t.appendChild(e),F.tabs.length>=2){let i=document.createElement("button");i.className="binder-close",i.textContent="\\xD7",i.setAttribute("aria-label",`Close ${u.title}`),i.addEventListener("click",s=>{s.stopPropagation(),p(D)}),t.appendChild(i)}return t.addEventListener("click",()=>{g(D)}),t}function g(u){let D=E();if(u<0||u>=D.tabs.length)return;let n=D.tabs[u];if(!n)return;D.activeIndex=u,C(D);let F=new URL(o(n.slug),window.location.origin);window.spaNavigate?window.spaNavigate(F,!1):window.location.href=F.toString()}function p(u){let D=E();if(D.tabs.length<2)return;let n=u===D.activeIndex;D.tabs.splice(u,1),n?(D.activeIndex=Math.min(u,D.tabs.length-1),C(D),g(D.activeIndex)):(u<D.activeIndex&&D.activeIndex--,C(D),a())}var l=null;function b(){let u=document.getElementById("stacked-pages-container");if(!u)return;let D=d(),n=m();if(D===l){A(u,E());return}l=D;let F=E(),r=f(u),t=F.tabs.findIndex(e=>e.slug===D);if(t>=0)F.tabs[t].title=n,F.activeIndex=t;else{let e={slug:D,title:n},i=F.activeIndex+1;for(F.tabs.splice(i,0,e),F.activeIndex=i;F.tabs.length>r.maxTabs;)F.activeIndex>0?(F.tabs.shift(),F.activeIndex--):F.tabs.pop()}C(F),A(u,F)}function a(){let u=document.getElementById("stacked-pages-container");u&&A(u,E())}function v(){b();let u=()=>a();window.addEventListener("resize",u),window.addCleanup&&window.addCleanup(()=>window.removeEventListener("resize",u))}typeof document<"u"&&(document.addEventListener("nav",()=>{v()}),document.addEventListener("render",()=>{a()}));\n';
